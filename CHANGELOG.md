...
- feat: `#mpcfill --blur PX` applies a Gaussian blur symmetrically to the Scryfall reference and every candidate MPCFill render before CLIP embedding. Use when matching a grainy / halftone-pattern scan that otherwise tricks the matcher into picking a similarly grainy low-res candidate over the clean one. 1–2 px is typically enough.
