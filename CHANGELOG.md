...
- feat: `#mpcfill --retro` modeline option restricts MPCFill candidate selection to sources whose `source_name` matches retro-frame keywords (`retro`, `old border`, `1993`, `1997`, `classic`, `vintage`, case-insensitive substring). Useful for cards with no real retro Scryfall print — community-rendered old-frame variants get picked instead of the modern default. Falls back to the full candidate set when no retro-named source exists so the card isn't silently dropped. The keyword filter narrows BEFORE the LightGlue keypoint matcher runs.
- feat: `--prefer-retro-frame` on `mtg-proxies convert` boosts pre-2015 frames (1993 / 1997 / 2003) in `recommend_print` hard enough to beat the default highres-2015 pick when a retro reprint exists (Brothers' War Retro, Mystery Booster old-frame, Time Spiral Remastered, etc.). Falls back silently to the regular winner when no retro candidate exists for a card. Threads through file decklists, Manastack, and Archidekt parsers.
