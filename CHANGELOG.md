...
- feat: `#mpcfill --identifier <ID>` locks in a specific MPCFill render by its backend Identifier (Google Drive file ID), bypassing the auto-matcher entirely. Use when the matcher consistently picks the wrong candidate. New companion subcommand `mtg-proxies mpcfill-pick "Card Name"` opens an interactive Tkinter thumbnail picker so users can manually choose the right render and copy the printed Identifier into their decklist.
