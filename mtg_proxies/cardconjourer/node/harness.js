...
function reg(file, opts) {
    // ``opts`` is either a string (legacy: just family) or a node-canvas
    // registerFont opts object with {family, weight?, style?}. Using the
    // explicit weight/style form is what lets Windows resolve "MPlantin-Italic"
    // automatically as "MPlantin" + italic — node-canvas's CSS font parser
    // splits hyphenated names like "MPlantin-Italic" / "Matrix-Bold" into a
    // base family + style/weight modifier, so we have to register each
    // variant as that modifier on the base family.
    if (typeof opts === 'string') opts = { family: opts };
    ...
    // binary on checkout (the `* text=auto` problem).
