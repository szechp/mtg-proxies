...
def _run_completion(args: argparse.Namespace) -> None:
    ...
    if tokens:
        # weight a forced theme at the WEAKEST of the cube's existing tribes (or the min lane) so it
        # comes in as a secondary lane — never above the established main themes.
        tribes = [w for k, w in lanes.items() if k.startswith("subtype:")]
        ref = min(tribes or list(lanes.values())) if lanes else 20.0
        forced = theme_lanes(tokens, weight=ref)
        # for an already-derived theme, keep its (stronger) derived weight; only lift unseen ones.
        lanes.update({sig: max(w, lanes.get(sig, 0.0)) for sig, w in forced.items()})
        print(f"Forced themes at weight {ref:.0f} (secondary to existing tribes): "
